...
GRADE_WEIGHT = {"최우수": 1.25, "우수": 1.10, "일반": 1.00}
DEDICATED_WEIGHT = {"Y": 1.20, "N": 1.00}
...
def _w_grade(grade: str) -> float:
    return float(GRADE_WEIGHT.get(str(grade), 1.0))


def _w_dedicated(flag: str) -> float:
    return float(DEDICATED_WEIGHT.get(str(flag), 1.0))
...
def generate_actions_for_row(
    ...
    w = _w_grade(grade) * _w_dedicated(dedicated)
    ...
    if util >= 0.7 and net < 0:
        score = (util - 0.7) * 10.0 * w + sev_w * 3.0
